const MENU = [
  ...
  for (let i = 0; i < MENU.length; i++) {
    ...
    li.setAttribute("class", "item")   li.appendChild(document.createTextNode(MENU[i].title))
    ul.appendChild(li);
    
    // strOpt = MENU[i].options + " : ";
    for (let j = 0; j < MENU[i].options.length; j++) {
      // str += MENU[i].options[j].url + ",";
      // strOpt += MENU[i].options[j].opt + ",";
    }
